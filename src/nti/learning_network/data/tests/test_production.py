...
def _get_assessed_question_set():
    assessed_parts = []
    assessed = []
    assessed_parts.append(QAssessedPart(submittedResponse=_response,
                                        assessedValue=1.0))
    assessed.append(QAssessedQuestion(questionId=_question_id,
                                      parts=assessed_parts))
    return QAssessedQuestionSet(questionSetId=_question_set_id,
                                questions=assessed)
...
def _get_history_item(assignment_type=QAssignment):
    ...
    qs_submission = QuestionSetSubmission(
        ...
        questions=(QuestionSubmission(questionId=_question_id, 
									  parts=(_response,)),)
    )
    submission = AssignmentSubmission(assignmentId=_assignment_id,
                                      parts=(qs_submission,))
...
class TestProduction(LearningNetworkTestCase):
    ...
    @WithMockDSTrans
    def _get_user(self):
        return User.create_user(username=u'new_user1')
    ...
    def test_assignment_stats(self, mock_get_assignments):
        ...
        # Dupe, late
        assignment3 = _get_assignment(assignment_id, user,
        ...
        # Timed, late
        assignment_id3 = u'assignment3'
        submission, _ = _get_history_item(QTimedAssignment)
        assignment4 = _get_assignment(assignment_id3, user,
                                      submission, is_late=True)
    ...
    def test_highlight_stats(self, mock_get_highlights):
        ...
        assert_that(self.stat_source.HighlightStats.Count,
                    is_(len(highlights)))
